...
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu') #原：device = torch.device('cuda:0')
    ...
    with trange(FLAGS.total_steps, dynamic_ncols=True) as pbar:
        for step in pbar:
            ...
            if FLAGS.sample_step > 0 and step % FLAGS.sample_step == 0:
                print(f"Step {step}: Attempting to sample and save image...")  # 添加这行
                net_model.eval()
                with torch.no_grad():
                    x_0 = ema_sampler(x_T)
    ...
    with torch.no_grad():
        ...
        for i in trange(0, FLAGS.num_images, FLAGS.batch_size, desc=desc):
            ...
            for j in range(batch_size):
                img_path = os.path.join(sample_dir, f'image_{i + j:05d}.png')
                save_image((batch_images[j] + 1) / 2, img_path)
                print(f"Saved image to: {img_path}")

            images.append((batch_images + 1) / 2)
        images = torch.cat(images, dim=0).numpy()

    print(f"Finished generating {FLAGS.num_images} images!")
    # 不再计算FID
